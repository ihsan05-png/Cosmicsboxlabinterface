// AES-128 Implementation with custom S-Box support

// Default AES S-Box (can be replaced with custom)
const DEFAULT_SBOX = [
  
];

// Round constants
const RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];

// Galois Field multiplication
function gmul(a: number, b: number): number {
  let p = 0;
  for (let i = 0; i < 8; i++) {
    if (b & 1) p ^= a;
    const hiBitSet = a & 0x80;
    a <<= 1;
    if (hiBitSet) a ^= 0x1b;
    b >>= 1;
  }
  return p & 0xFF;
}

// Create inverse S-Box
function createInvSBox(sbox: number[]): number[] {
  const invSBox = new Array(256);
  for (let i = 0; i < 256; i++) {
    invSBox[sbox[i]] = i;
  }
  return invSBox;
}

// Key expansion
function keyExpansion(key: number[], sbox: number[]): number[][] {
  const w: number[][] = [];
  
  // First 4 words are the key itself
  for (let i = 0; i < 4; i++) {
    w[i] = [key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]];
  }
  
  // Generate remaining words
  for (let i = 4; i < 44; i++) {
    let temp = [...w[i - 1]];
    
    if (i % 4 === 0) {
      // RotWord
      temp = [temp[1], temp[2], temp[3], temp[0]];
      // SubWord
      temp = temp.map(b => sbox[b]);
      // XOR with Rcon
      temp[0] ^= RCON[(i / 4) - 1];
    }
    
    w[i] = [];
    for (let j = 0; j < 4; j++) {
      w[i][j] = w[i - 4][j] ^ temp[j];
    }
  }
  
  return w;
}

// SubBytes transformation
function subBytes(state: number[][], sbox: number[]): void {
  for (let i = 0; i < 4; i++) {
    for (let j = 0; j < 4; j++) {
      state[i][j] = sbox[state[i][j]];
    }
  }
}

// ShiftRows transformation
function shiftRows(state: number[][]): void {
  let temp: number;
  
  // Row 1: shift left by 1
  temp = state[1][0];
  state[1][0] = state[1][1];
  state[1][1] = state[1][2];
  state[1][2] = state[1][3];
  state[1][3] = temp;
  
  // Row 2: shift left by 2
  temp = state[2][0];
  state[2][0] = state[2][2];
  state[2][2] = temp;
  temp = state[2][1];
  state[2][1] = state[2][3];
  state[2][3] = temp;
  
  // Row 3: shift left by 3 (or right by 1)
  temp = state[3][3];
  state[3][3] = state[3][2];
  state[3][2] = state[3][1];
  state[3][1] = state[3][0];
  state[3][0] = temp;
}

// Inverse ShiftRows
function invShiftRows(state: number[][]): void {
  let temp: number;
  
  // Row 1: shift right by 1
  temp = state[1][3];
  state[1][3] = state[1][2];
  state[1][2] = state[1][1];
  state[1][1] = state[1][0];
  state[1][0] = temp;
  
  // Row 2: shift right by 2
  temp = state[2][0];
  state[2][0] = state[2][2];
  state[2][2] = temp;
  temp = state[2][1];
  state[2][1] = state[2][3];
  state[2][3] = temp;
  
  // Row 3: shift right by 3
  temp = state[3][0];
  state[3][0] = state[3][1];
  state[3][1] = state[3][2];
  state[3][2] = state[3][3];
  state[3][3] = temp;
}

// MixColumns transformation
function mixColumns(state: number[][]): void {
  for (let c = 0; c < 4; c++) {
    const s0 = state[0][c];
    const s1 = state[1][c];
    const s2 = state[2][c];
    const s3 = state[3][c];
    
    state[0][c] = gmul(s0, 2) ^ gmul(s1, 3) ^ s2 ^ s3;
    state[1][c] = s0 ^ gmul(s1, 2) ^ gmul(s2, 3) ^ s3;
    state[2][c] = s0 ^ s1 ^ gmul(s2, 2) ^ gmul(s3, 3);
    state[3][c] = gmul(s0, 3) ^ s1 ^ s2 ^ gmul(s3, 2);
  }
}

// Inverse MixColumns
function invMixColumns(state: number[][]): void {
  for (let c = 0; c < 4; c++) {
    const s0 = state[0][c];
    const s1 = state[1][c];
    const s2 = state[2][c];
    const s3 = state[3][c];
    
    state[0][c] = gmul(s0, 14) ^ gmul(s1, 11) ^ gmul(s2, 13) ^ gmul(s3, 9);
    state[1][c] = gmul(s0, 9) ^ gmul(s1, 14) ^ gmul(s2, 11) ^ gmul(s3, 13);
    state[2][c] = gmul(s0, 13) ^ gmul(s1, 9) ^ gmul(s2, 14) ^ gmul(s3, 11);
    state[3][c] = gmul(s0, 11) ^ gmul(s1, 13) ^ gmul(s2, 9) ^ gmul(s3, 14);
  }
}

// AddRoundKey
function addRoundKey(state: number[][], roundKey: number[][]): void {
  for (let c = 0; c < 4; c++) {
    for (let r = 0; r < 4; r++) {
      state[r][c] ^= roundKey[c][r];
    }
  }
}

// Convert string to state matrix
function textToState(text: string): number[][] {
  const bytes = new TextEncoder().encode(text.padEnd(16, '\0').substring(0, 16));
  const state: number[][] = [[], [], [], []];
  
  for (let c = 0; c < 4; c++) {
    for (let r = 0; r < 4; r++) {
      state[r][c] = bytes[c * 4 + r];
    }
  }
  
  return state;
}

// Convert state matrix to hex string
function stateToHex(state: number[][]): string {
  const bytes: number[] = [];
  for (let c = 0; c < 4; c++) {
    for (let r = 0; r < 4; r++) {
      bytes.push(state[r][c]);
    }
  }
  return bytes.map(b => b.toString(16).padStart(2, '0')).join('');
}

// Convert hex string to state matrix
function hexToState(hex: string): number[][] {
  const bytes = hex.match(/.{1,2}/g)?.map(h => parseInt(h, 16)) || [];
  const state: number[][] = [[], [], [], []];
  
  for (let c = 0; c < 4; c++) {
    for (let r = 0; r < 4; r++) {
      state[r][c] = bytes[c * 4 + r] || 0;
    }
  }
  
  return state;
}

// AES Encryption
export function aesEncrypt(plaintext: string, keyStr: string, customSBox?: number[]): string {
  const sbox = customSBox || DEFAULT_SBOX;
  
  // Prepare key (128-bit = 16 bytes)
  const keyBytes = new TextEncoder().encode(keyStr.padEnd(16, '\0').substring(0, 16));
  const key = Array.from(keyBytes);
  
  // Key expansion
  const roundKeys = keyExpansion(key, sbox);
  
  // Initialize state
  const state = textToState(plaintext);
  
  // Initial round
  addRoundKey(state, roundKeys.slice(0, 4));
  
  // Main rounds (9 rounds for AES-128)
  for (let round = 1; round < 10; round++) {
    subBytes(state, sbox);
    shiftRows(state);
    mixColumns(state);
    addRoundKey(state, roundKeys.slice(round * 4, (round + 1) * 4));
  }
  
  // Final round (no MixColumns)
  subBytes(state, sbox);
  shiftRows(state);
  addRoundKey(state, roundKeys.slice(40, 44));
  
  return stateToHex(state);
}

// AES Decryption
export function aesDecrypt(ciphertext: string, keyStr: string, customSBox?: number[]): string {
  const sbox = customSBox || DEFAULT_SBOX;
  const invSBox = createInvSBox(sbox);
  
  // Prepare key
  const keyBytes = new TextEncoder().encode(keyStr.padEnd(16, '\0').substring(0, 16));
  const key = Array.from(keyBytes);
  
  // Key expansion
  const roundKeys = keyExpansion(key, sbox);
  
  // Initialize state
  const state = hexToState(ciphertext);
  
  // Initial round
  addRoundKey(state, roundKeys.slice(40, 44));
  
  // Main rounds (reverse order)
  for (let round = 9; round > 0; round--) {
    invShiftRows(state);
    subBytes(state, invSBox);
    addRoundKey(state, roundKeys.slice(round * 4, (round + 1) * 4));
    invMixColumns(state);
  }
  
  // Final round
  invShiftRows(state);
  subBytes(state, invSBox);
  addRoundKey(state, roundKeys.slice(0, 4));
  
  // Convert state to text
  const bytes: number[] = [];
  for (let c = 0; c < 4; c++) {
    for (let r = 0; r < 4; r++) {
      bytes.push(state[r][c]);
    }
  }
  
  return new TextDecoder().decode(new Uint8Array(bytes)).replace(/\0+$/, '');
}
